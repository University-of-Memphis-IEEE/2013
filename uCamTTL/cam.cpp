 ...
 * @return True if successful, false otherwise.
 */
boolean Cam::initial( ColorType colorType, PreviewResolution previewResolution, JPEGResolution jpegResolution )
{
 ...
 * @return True if successful, false otherwise
 */
boolean Cam::setLightFrequency( FrequencyType frequencyType )
{
  createCommand( CMD_LIGHTFREQ, (uint8_t)frequencyType, 0, 0, 0 );
  sendCommand();
 ...
 * @return True if successful, false otherwise
 */
boolean Cam::setPackageSize( uint16_t size )
{
  createCommand( CMD_PACKAGESIZE, 0x08, (uint8_t)(size & 0xFF), (uint8_t)(size >> 8), 0 );
  sendCommand();
 ...
 * @return True if successful, false otherwise
 */
boolean Cam::snapshot( SnapshotType snapshotType, uint16_t skipFrames )
{
  createCommand( CMD_SNAPSHOT, snapshotType, (uint8_t)(skipFrames & 0xFF), (uint8_t)(skipFrames >> 8), 0 ); 
  sendCommand();
 ...
 * @return True if successful, false otherwise
 */
boolean Cam::getJPEGPicture( PictureType pictureType, uint16_t processDelay, void (*callback)( uint16_t picSize, uint16_t packPicDataSize, uint16_t packCount, uint8_t* pack ) )
{
  ...
  uint8_t package[_packageSize];
  boolean packageSuccess;
 ...
 * @return True if successful, false otherwise
 */
boolean Cam::reset( boolean completeReset )
{
 ...
 * @return True if successful, false otherwise
 */
boolean Cam::setBaudRate( BaudRate baudRate )
{
  createCommand( CMD_BAUDRATE, (uint8_t)baudRate, 0x01, 0, 0 );
  sendCommand();
 ...
 * @return True if successful, false otherwise
 */
boolean Cam::getRawPicture( PictureType pictureType, uint8_t pictureBuffer[], uint16_t bufferSize, uint16_t processDelay )
{
  uint16_t pictureSize = 0;
 ...
 */
boolean Cam::getPicture( PictureType pictureType, uint16_t processDelay, uint16_t &pictureSize )
{
  pictureSize = 0;
 ...
 */
boolean Cam::waitForACK( uint32_t timeout, uint8_t cmdId )
{
  boolean success = waitForResponse( timeout );
 ...
 */
void Cam::createCommand( const uint8_t cmd, uint8_t param1, uint8_t param2, uint8_t param3, uint8_t param4 )
{
 ...
 * Sends and ACK.
 */
void Cam::sendACK( const uint8_t cmd )
{
  sendACK( cmd, 0 );
}
 ...
 */
void Cam::sendACK( const uint8_t cmd, uint16_t packageId )
{
  createCommand( CMD_ACK, cmd, 0, (uint8_t)(packageId & 0xFF), (uint8_t)(packageId >> 8) );
  sendCommand();
}
 ...
 */
boolean Cam::waitForResponse( uint32_t timeout )
{
 ...
 * @return True if response received, false otherwise
 */
boolean Cam::waitForResponse( uint32_t timeout, uint8_t buffer[], uint16_t bufferLength )
{
  uint8_t byteCnt = 0;
...
uint8_t Cam::serial_available(void)
{
	uint8_t ret;
	if(_serialPort){
...
void Cam::serial_print(uint8_t data)
{
	if(_serialPort){
